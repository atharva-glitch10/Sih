...
%   MathWorks SIH - Explainable AI for DR Screening in Rural PHCs
    ...
    if nvDetected || (features.nvArea > 50)
        ...
        ruleExplanation = sprintf('Grade 4 (Proliferative DR): %s', reasons{1});
        isReferable = true;
        return;
    end
    ...
    if (quadsWithSevereHMs >= 4) || (quadsWithHMs == 4 && hmCount >= 18)
        ...
        ruleExplanation = sprintf('Grade 3 (Severe NPDR): %s', reasons{1});
        isReferable = true;
        return;
    end
    ...
    if (hmCount >= 3) || (heArea > 30) || (maCount >= 5 && quadsWithHMs >= 2) || (dmeRatio < 1.0 && heArea > 0)
        ...
        ruleExplanation = sprintf('Grade 2 (Moderate NPDR): %s', strjoin)(reasons, ' | ');
        isReferable = true;
        return;
    end
    ...
    if (maCount >= 1) || (hmCount == 1 && heArea == 0)
        ...
        ruleExplanation = sprintf('Grade 1 (Mild NPDR): %s', reasons{1});
    ...
    ruleExplanation = sprintf('Grade 0 (No DR): %s', reasons{1});
    isReferable = false;
end
